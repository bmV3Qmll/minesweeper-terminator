...
def boundary(state):
	# Return a binary mask marking all squares that are adjacent to a number.
...
class MinesweeperSolver:
	...
	def _csp(self, state):
		'''
		Find all unknown cells which are neighbors of some numbers (boundary of unknown islands).
		Divide them into connected components: two cells are adjacent if at least one number put a constraint on them.
		Next, divide each component into areas in which every cells in it are equally constrained 
		and must therefore have the same probabilities.
		The combinations of the number of mines in these areas are computed with constraint programming.
		Count the number of models in which each area has the given number of mines.
		Calculate the average expected number of mines per square in a component if it has M mines.
		Finally, combine to form the final probabilities.
		'''
		...
		def _relative_weight(range_no_mines_comps, no_unconstrained):
			...
			for no_mines_comps in range_no_mines_comps:
				no_mines_unconstrained_next = mines_left - no_mines_comps
				for no_mines_unconstrained in range(no_mines_unconstrained + 1, no_mines_unconstrained_next + 1):
					...
					if n == no_mines_unconstrained:
						weight /= n
					else:
						weight = weight * (n - no_mines_unconstrained) / no_mines_unconstrained
				rel_no_unconstrained_models[no_mines_comps] = weight
				if no_mines_unconstrained >= n:
					break
		...
		def _combine_components(state, prob, no_models_per_comp, comp_prob_per_comp):
			...
			if comp_prob_per_comp:
				...
				for mines_combination in product(*[comp.keys() for comp in no_models_per_comp]):
					...
					if no_mines_left - n <= no_mines_comp <= no_mines_left:
						# Combine the prob arrays for this component combination.
						combined_prob = reduce(np.add, [comp_prob_per_comp[comp][no_mines_comp]
							for comp, no_mines_comp in enumerate(mines_combination)])
						combined_no_models = reduce(mul, [no_models_per_comp[comp][no_mines_comp]
							for comp, no_mines_comp in enumerate(mines_combination)])
		...
		for c in range(1, no_components + 1):
			...
			problem.addConstraint(MaxSumConstraint(mines_left()), list(areas.values()))
		...
		prob = _combine_components(state, prob, no_models_per_comp, comp_prob_per_comp)
